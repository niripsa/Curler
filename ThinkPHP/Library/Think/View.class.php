...
class View {
    ...
    public function parseTemplate($template='') {
        ...
        // 获取当前模块
        $module   =  MODULE_NAME;
        #↑Kekehome或者Home这些文件夹的名字
        #这些常量是在加载Controller时定义的
        if(strpos($template,'@')){ // 跨模块调用模版文件
            list($module,$template)  =   explode('@',$template);
        }
        // 获取当前主题的模版路径
        defined('THEME_PATH') or    define('THEME_PATH', $this->getThemePath($module));
        #THEME_PATH="./Application/Kekehome/View/""
        ...
        if('' == $template) {
            ...
            //如果strpos返回false,说明字符串中不存在指定字符"/""
            $template = CONTROLLER_NAME . $depr . $template;
        }
        $file   =   THEME_PATH.$template.C('TMPL_TEMPLATE_SUFFIX');
        #C('TMPL_TEMPLATE_SUFFIX')即".html"
